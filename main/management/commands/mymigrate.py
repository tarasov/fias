...
class Command(BaseCommand):
    ...
    manage_py = os.path.join(settings.PROJECT_PATH, 'manage.py')
    ...
    def delete(self, app):
        migration_dir = os.path.join(settings.PROJECT_PATH, app, 'migrations')
        print migration_dir
        shutil.rmtree(migration_dir, ignore_errors=True, onerror=None)

    def handle(self, *args, **options):
        myapps = []
        for app in os.listdir(settings.PROJECT_PATH):
        ...
        if options['initial']:
            ...
            print u'Удаляем папки с миграциями...'
            for app in myapps:
                self.delete(app)
            print u'Удаляем записи о миграциях из БД...'
            MigrationHistory.objects.all().delete()
            print u'Теперь можно создать инициальные миграции!'